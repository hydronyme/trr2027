...
private:
 int init_gstreamer() {
         ...
         std::string pipeline_file_str =
            ...
            "imagefreeze ! "
            "videoconvert ! "
            "video/x-raw,format=BGR,framerate=60/1 ! "
  ...
  void initVPI() {
    ...
    // Allocate CUDA pitch
    CHECK_CUDA_STATUS(cudaMalloc(&cudaIn, 3 * width * height * sizeof(uint8_t)));
    ...
    */
    ...
    CHECK_VPI(vpiImageCreate(width, height, VPI_IMAGE_FORMAT_BGR8, VPI_BACKEND_CUDA, &vpiIn));
  ...
  VPIImageData GetGenericPLData(int width, int height, VPIImageBufferType bufferType, VPIByte *pBase) {
    VPIImageBufferPitchLinear dataIn = {};
    dataIn.format = VPI_IMAGE_FORMAT_U8;
    dataIn.numPlanes = 1;
    dataIn.planes[0].width = width;
    dataIn.planes[0].height = height;
    dataIn.planes[0].pitchBytes = sizeof(uint8_t) * width;
    dataIn.planes[0].pixelType = VPI_PIXEL_TYPE_INVALID;
    dataIn.planes[0].offsetBytes = 0;
    dataIn.planes[0].pBase = pBase;
    VPIImageData vpiImgData = {};
    vpiImgData.bufferType = bufferType;
    vpiImgData.buffer.pitch = dataIn;
    return vpiImgData;
  }
  ...
  void writeCudaImg() {
    cv::Mat canny(height, width, CV_8UC1);
    CHECK_CUDA_STATUS(cudaMemcpy(canny.data, cudaCanny, width * height * sizeof(uint8_t), cudaMemcpyDeviceToHost)); 
    imwrite("canny.png", canny);

    cv::Mat gray(height, width, CV_8UC1);
    CHECK_CUDA_STATUS(cudaMemcpy(gray.data, cudaGray, width * height * sizeof(uint8_t), cudaMemcpyDeviceToHost)); 
    imwrite("gray.png", gray);
  }

  /*
  void copyToCudaMem(cv::Mat img, VPIImage vpiIn, VPIImageData dataIn) {

    vpiImageLockData(vpiIn, VPI_LOCK_WRITE, VPI_IMAGE_BUFFER_CUDA_PITCH_LINEAR, &dataIn);

    // Extraction de l'adresse GPU Device pour une image CUDA_PITCH_LINEAR à un
    // seul plan (BGR8) :
    uint8_t *cudaIn = dataIn.buffer.pitch.planes[0].pBase;
    size_t dst_pitch = dataIn.buffer.pitch.planes[0].pitchBytes;

    // Copie 2D de la RAM Host (cv::Mat CPU) vers la RAM GPU (d_ptr)
    size_t widthbytes = img.cols * img.elemSize(); // Largeur en octets (cols * 3)
    size_t src_pitch = img.step;   // Pitch de l'image CPU

    cudaMemcpy2D(cudaIn,     // Destination : Pointeur GPU VPI
                 dst_pitch,  // Alignement (Pitch) GPU
                 img.data,   // Source : Pointeur CPU OpenCV
                 src_pitch,  // Pitch CPU
                 widthbytes, // Largeur d'une ligne
  ...
  bool createVPIImageDataFromGstBuffer(GstBuffer *gstBuffer) {
    ...
    dataIn.bufferType = VPI_IMAGE_BUFFER_HOST_PITCH_LINEAR;
    //dataIn.bufferType = VPI_IMAGE_BUFFER_CUDA_PITCH_LINEAR;
    dataIn.buffer.pitch.format = VPI_IMAGE_FORMAT_BGR8;
    dataIn.buffer.pitch.numPlanes = 1;
    dataIn.buffer.pitch.planes[0].pBase = (VPIByte*) map.data;
    dataIn.buffer.pitch.planes[0].width = width;
    dataIn.buffer.pitch.planes[0].height = height;
    dataIn.buffer.pitch.planes[0].pitchBytes = 3 * sizeof(uint8_t) * width;
    dataIn.buffer.pitch.planes[0].offsetBytes = 0;

    timer_2= cv::getTickCount();
    CHECK_VPI(vpiImageCreateWrapper(&dataIn, &params, 0, &vpiIn));
    CHECK_VPI(vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CUDA, vpiIn, vpiGray, NULL));
    ...
    submitLineAnalysis(cudaGray, cudaCanny, width, height, cudaStream, d_result);
    CHECK_VPI(vpiStreamSync(stream));
    ...
    for (int i = 0; i < 720; i++) {
      if(h_result[i]!=-1) 
      {
        ...
        //cv::circle(bgr, cv::Point(h_result[i],i), 3, cv::Scalar( 255, 0, 0 ),cv::FILLED,cv::LINE_8);
      }
    }
...
GstFlowReturn CameraAnalysis::onNewSample( GstAppSink* appsink, gpointer user_data) {
    ...
    camera->createVPIImageDataFromGstBuffer(buffer);
    return GST_FLOW_OK;
}
