 ...
 int init_gstreamer() {
  ...
  void initVPI() {
    ...
    */
    ...
    CHECK_VPI(vpiImageCreate(width, height, VPI_IMAGE_FORMAT_NV12, VPI_BACKEND_CUDA, &vpiIn));
  ...
  bool createVPIImageDataFromGstBuffer(GstBuffer *gstBuffer,GstCaps *caps) {
   ...
   if (!gst_video_info_from_caps(&info, caps))
        throw std::runtime_error("Impossible de récupérer GstVideoInfo");

    if (GST_VIDEO_INFO_FORMAT(&info) != GST_VIDEO_FORMAT_NV12)
        throw std::runtime_error("Le buffer n'est pas en NV12");

    const uint32_t width  = GST_VIDEO_INFO_WIDTH(&info);
    const uint32_t height = GST_VIDEO_INFO_HEIGHT(&info);
    const uint32_t stride = GST_VIDEO_INFO_PLANE_STRIDE(&info, 0);

    GstVideoFrame frame{};

    if (!gst_video_frame_map( &frame, &info, gstBuffer, GST_MAP_READ))
    {
        throw std::runtime_error("gst_video_frame_map failed");
    }

    std::cout
    << "format     = " << GST_VIDEO_INFO_NAME(&info)
    << "\nwidth      = " << GST_VIDEO_INFO_WIDTH(&info)
    << "\nheight     = " << GST_VIDEO_INFO_HEIGHT(&info)
    << "\nplanes     = " << GST_VIDEO_INFO_N_PLANES(&info)
    << "\nY stride   = " << GST_VIDEO_FRAME_PLANE_STRIDE(&frame, 0)
    << "\nUV stride  = " << GST_VIDEO_FRAME_PLANE_STRIDE(&frame, 1)
    << "\nY ptr      = " << GST_VIDEO_FRAME_PLANE_DATA(&frame, 0)
    << "\nUV ptr     = " << GST_VIDEO_FRAME_PLANE_DATA(&frame, 1)
    << std::endl;
    ...
    */
    ...
    //CHECK_VPI(vpiImageCreateWrapper(&dataIn,nullptr, VPI_BACKEND_CUDA, &vpiGray));
    ...
    if (nb_loop==22) {
        ...
        //writeCudaImg((uint8_t *)map.data,"gray.png"); 

        cv::Mat imgGray( height, width, CV_8UC1, map.data, width);
        cv::imwrite("gray.png", imgGray);

        //cv::Mat imgCanny( height, width, CV_8UC1, dataCanny.buffer.pitch.planes[0].pBase, width);
        //cv::imwrite("canny.png", imgCanny);
